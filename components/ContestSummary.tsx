...
export const ContestSummary: React.FC<Props> = ({ 
  ...
  const getVerdict = () => {
    ...
    if ((slateStats?.totalPlayers || 0) === 0) return { status: 'WARN', label: 'No Reference Data', color: 'amber' };

    const hasManyWarnings = (slateStats?.warnings?.length || 0) >= 3;
  ...
  const packDisplayName = referenceMeta?.files?.pack_name || (referencePackPath ? referencePackPath.split('/').pop() : null) || 'Unknown pack';
        ...
        <div className="bg-white dark:bg-charcoal-card rounded-xl shadow-sm border border-cloud-darker dark:border-charcoal-card overflow-hidden">
            ...
            <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div><div className="text-xs text-gray-500 uppercase font-medium">Est. Payouts</div><div className="font-bold text-cloud-text dark:text-white">~{derived.estimatedPaidPlaces.toLocaleString()} Places</div></div>
                </div>
            </div>
        </div>
